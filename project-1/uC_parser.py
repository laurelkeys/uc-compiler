...
class UCParser:
    ...
    def __init__(self):
        self.lexer = UCLexer(
            error_func=lambda msg, x, y: print(
    ...
    def _build_declarations(self, spec, decls):
        ...
        for decl in decls:
            ...
            declaration = Decl(
                None, decl["decl"], decl.get("init"), coord=decl["decl"].coord
            )
    ...
    def _build_function_definition(self, spec, decl, param_decls, body):
        ...
        return FuncDef(spec, declaration, param_decls, body, coord=decl.coord)
    ...
    def p_function_definition(self, p):
        ...
        if p[1] is not None:
            ...
            spec = dict(
                type=[
                    Type(_default_function_return_type, coord=self._token_coord(p, 1))
                ],
                function=[],
            )
        p[0] = self._build_function_definition(
            spec, decl=p[2], param_decls=p[3], body=p[4]
        )
    ...
    def p_declaration(self, p):
        ...
        if p[2] is None:
            p[0] = self._build_declarations(
                spec=p[1], decls=[dict(decl=None, init=None)]
            )
        else:
    ...
    def p_declaration__list(self, p):
        ...
        )  # NOTE declaration is already a list
