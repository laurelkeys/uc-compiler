    ...
    def visit_ArrayRef(self, node: ArrayRef):  # [name*, subscript*]
        ...
        if node.attrs.get("child?", False):
            ...
            if isinstance(node.name, ArrayRef):
                ...
                self.emit_op(
                    "+", TYPE_INT, left=node.name.attrs["reg"], right=_target, target=_fixed_subs,
                )
                _target = _fixed_subs
        else:
    ...
    def visit_UnaryOp(self, node: UnaryOp):  # [op, expr*]
        ...
        if node.op == "+":
            ...
            self.emit_store(  # update value
                _type=_expr_type, source=_unop_target, target=self.fregisters[node.expr.name]
            )
