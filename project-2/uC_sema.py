...
class Visitor(NodeVisitor):
    ...
    def visit_ArrayRef(self, node: ArrayRef):  # [name*, subscript*]
        ...
        if _name_type[0] == TYPE_ARRAY:
            ...
            assert False, f"Type {_name_type} doesn't support array-like indexing{node.coord}"

    def visit_Assert(self, node: Assert):  # [expr*]
        self.visit(node.expr)
        _expr_type = node.expr.attrs['type']
        assert _expr_type == [TYPE_BOOL], f"No implementation for: `assert {_expr_type}`{node.coord}"
    ...
    def visit_Assignment(self, node: Assignment):  # [op, lvalue*, rvalue*]
        ...
        if isinstance(node.lvalue, ID):
            ...
            assert False, f"Assignment to invalid lvalue `{type(node.lvalue)}`{node.coord}"

        assert _lname in self.symtab.current_scope, f"Assignment to unknown lvalue `{_lname}`{node.coord}"
        ...
        if _rname is not None:
            assert _rname in self.symtab.current_scope, (
                f"Assignment of unknown rvalue: `{_lname}` = `{_rname}`{node.coord}")

        assert _ltype == _rtype, f"Type mismatch: `{_ltype} {node.op} {_rtype}`{node.coord}"

        assert node.op in uC_ops.assign_ops, (
            f"Unexpected operator in adssignment operation: `{node.op}`{node.coord}")

        assert node.op in _ltype[0].assign_ops, (  # use the "outermost" type
            f"Operation not supported by type {_ltype}: `{_ltype} {node.op} {_rtype}`{node.coord}")
    ...
    def visit_BinaryOp(self, node: BinaryOp):  # [op, left*, right*]
        ...
        for i, _operand in enumerate([node.left, node.right]):
            if isinstance(_operand, ID):
                ...
                assert _operand_name in self.symtab.current_scope, (
                    f"Identifier `{_operand_name}` not defined{node.coord}")
        ...
        if node.op in uC_ops.binary_ops:
            ...
            assert False, f"Unexpected operator in binary operation: `{node.op}`{node.coord}"
    ...
    def visit_Break(self, node: Break):  # []
        assert self.symtab.in_loop, f"Break outside a loop{node.coord}"
    ...
    def visit_Cast(self, node: Cast):  # [type*, expr*]
        ...
        assert _valid_cast, f"Cast from `{_src_type}` to `{_dst_type}` is not supported{node.coord}"
    ...
    def visit_Compound(self, node: Compound):  # [decls**, stmts**]
        ...
        if _parent is None:
            ...
            if isinstance(_parent, FuncDef):
                ...
                assert False, f"Unexpected type openning a compount statement: {type(_parent)}{node.coord}"
        ...
        if isinstance(node.type, VarDecl):
            ...
            if sym_name in self.symtab.local_scope:
                assert sym_attrs['type'] == self.symtab.lookup(sym_name)['type'], (
                    ...
                    f"{sym_attrs['type']} and {self.symtab.lookup(sym_name)['type']}{node.coord}")
                ...
                assert len(sym_attrs['param_types']) == len(_declared_param_types), (
                    ...
                    f"{len(_declared_param_types)} expected{node.coord}")
                for _new_type, _old_type in zip(sym_attrs['param_types'], _declared_param_types):
                    assert _new_type == _old_type, (
                        ...
                        f"{_new_type} passed, {_old_type} expected{node.coord}")

        else:
            assert False, f"Unexpected type {type(node.type)} for node.type{node.coord}"

        if node.init is None:
            if isinstance(node.type, ArrayDecl):
                assert sym_attrs['dim'] is not None, (  # NOTE this must be True for the uC language
                    f"Array dimensions for `{sym_name}` can't be infered at compile time{node.coord}")
            ...
            if isinstance(node.init, ID):
                ...
                if not (node.type.attrs.get('dim', None) is None or node.init.attrs.get('dim', None) is None):
                    assert node.type.attrs.get('dim', []) == node.init.attrs.get('dim', []), (
                        ...
                        f"{node.type.attrs.get('dim', [])} and {node.init.attrs.get('dim', [])}{node.coord}")
                else:
                    sym_attrs['dim'] = (
            ...
            assert init_type == sym_attrs['type'], (
                f"Implicit conversions are not supported: {sym_attrs['type']} = {init_type}{node.coord}")
    ...
    def visit_For(self, node: For):  # [init*, cond*, next*, body*]
        ...
        if node.cond is not None:
            ...
            assert node.cond.attrs['type'] == [TYPE_BOOL], (
                f"Condition should be a bool instead of {node.cond.attrs['type']}{node.coord}")
    ...
    def visit_FuncCall(self, node: FuncCall):  # [name*, args*]
        ...
        self.visit(node.name)
        _name = node.name.attrs['name']
        assert _name in self.symtab.current_scope, f"Function `{_name}` not defined{node.coord}"
        ...
        assert len(_param_types) == len(_passed_args), (
            ...
            f"{len(_passed_args)} passed, {len(_param_types)} expected{node.coord}")
        ...
        for _passed_arg, _param_type in zip(_passed_args, _param_types):
            ...
            assert passed_type == _param_type, (
                ...
                f"{passed_type} passed, {_param_type} expected{node.coord}")
    ...
    def visit_FuncDef(self, node: FuncDef):  # [spec*, decl*, body*]
        ...
        #                 pass
        # assert _has_return or TYPE_VOID in node.attrs['type'], f"Function `{node.attrs['name']}` has no return{node.coord}"
    ...
    def visit_InitList(self, node: InitList):  # [exprs**]
        ...
        for expr in node.exprs[1:]:
            ...
            assert _type == expr.attrs['type'], (
                f"Init list must have homogeneous types: {_type} and {expr.attrs['type']}{node.coord}")
        ...
        if isinstance(node.exprs[0], InitList):
            ...
            for expr in node.exprs[1:]:
                assert dim == expr.attrs['dim'], (
                    f"Init list must have homogeneous dim sizes: {dim} and {expr.attrs['dim']}{node.coord}")
            ...
            for _expr in _print_exprs:
                if isinstance(_expr, ID):
                    assert _expr.name in self.symtab.current_scope, (
                        f"Attempt to print unknown identifier: `{_expr.name}`{node.coord}")
                elif isinstance(_expr, ArrayRef):
                    assert _expr.attrs['name'] in self.symtab.current_scope, (
                        f"Attempt to print unknown array: `{_expr.attrs['name']}`{node.coord}")
                elif isinstance(_expr, FuncCall):
                    assert _expr.attrs['name'] in self.symtab.current_scope, (
                        f"Attempt to print the return of an unknown function: `{_expr.attrs['name']}`{node.coord}")
    ...
    def visit_Program(self, node: Program):  # [gdecls**]
        ...
        for _key, _item in self.symtab.local_scope.items():
            if _item['type'][0] == TYPE_FUNC:
                assert _item.get('defined?', False), f"Function declared but not defined{node.coord}"
    ...
    def visit_Read(self, node: Read):  # [expr*]
        ...
        for _expr in _read_exprs:
            if isinstance(_expr, ID):
                assert _expr.name in self.symtab.current_scope, (
                    f"Attempt to read into unknown identifier: `{_expr.name}`{node.coord}")
            elif isinstance(_expr, ArrayRef):
                assert _expr.attrs['name'] in self.symtab.current_scope, (
                    f"Attempt to read into unknown array: `{_expr.attrs['name']}`{node.coord}")
            else:
                assert False, f"Unexpected node type in read: {type(_expr)}{node.coord}"

    def visit_Return(self, node: Return):  # [expr*]
        assert self.symtab.in_func, f"Return outside a function{node.coord}"

        if node.expr is not None:
            self.visit(node.expr)
            assert node.expr.attrs['type'] == self.symtab.curr_func.attrs['type'][1:], (
                f"Returning {node.expr.attrs['type']} on a {self.symtab.curr_func.attrs['type'][1:]} function{node.coord}")
        else:
            assert [TYPE_VOID] == self.symtab.curr_func.attrs['type'][1:], (
                f"Returning {[TYPE_VOID]} on a {self.symtab.curr_func.attrs['type'][1:]} function{node.coord}")
    ...
    def visit_UnaryOp(self, node: UnaryOp):  # [op, expr*]
        assert node.op in uC_ops.unary_ops, f"Unexpected operator in unary operation: `{node.op}`{node.coord}"

        self.visit(node.expr)

        _operand_name = node.expr.attrs.get('name', None)
        if isinstance(node.expr, ID):
            assert _operand_name in self.symtab.current_scope, f"Identifier `{_operand_name}` not defined{node.coord}"
            _type = self.symtab.lookup(_operand_name)['type']

        elif isinstance(node.expr, FuncCall):
            assert _operand_name in self.symtab.current_scope, f"Function `{_operand_name}` not defined{node.coord}"
        ...
        assert node.op in _type[0].unary_ops, (  # use the "outermost" type
            ...
            + (
                f"`{node.op}{_type}`"
                if node.op[0] != 'p'
                else f"`{_type}{node.op[1:]}`{node.coord}"
            )
        )

        if "++" in node.op or "--" in node.op:
            assert isinstance(node.expr, (ID, ArrayRef)), f"Expression is not assignable{node.coord}"
