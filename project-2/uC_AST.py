...
class UCType:
    ...
    def __init__(self, typename, default, unary_ops=None, binary_ops=None, rel_ops=None, assign_ops=None):
        ...
        assert all(op in unary_ops for op in self.unary_ops)
        assert all(op in binary_ops for op in self.binary_ops)
        assert all(op in rel_ops for op in self.rel_ops)
        assert all(op in assign_ops for op in self.assign_ops)
